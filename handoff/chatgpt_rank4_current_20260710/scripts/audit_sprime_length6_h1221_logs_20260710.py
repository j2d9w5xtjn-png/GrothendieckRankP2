#!/usr/bin/env python3
"""Audit and aggregate the split H=(1,2,2,1) S' sweep logs.
...
to the 19 Gorenstein-liftable quotient rings and the ten fibre rows used by
``sprime_length6_h1221_type2_nineteen_stratified_20260710.py``.
...
DATE_TAG = "20260710"
SOLVER_BASENAME = "sprime_length6_h1221_type2_nineteen_stratified_20260710.py"
FINAL_MARKER = "DONE sprime_length6_h1221_type2_nineteen_stratified_20260710"

RINGS = (
    "q01", "q03", "q05", "q06", "q07", "q08", "q09", "q10",
    "q15", "q16", "q17", "q18", "q20", "q21", "q22", "q23",
    "q24", "q25", "q27",
)
XY_MODELS = (
...
FILENAME_RE = re.compile(
    rf"^sprime_length6_h1221_(q\d{{2}})_(xy|t4)_(.+)_i([123])"
    rf"(?:_([A-Za-z0-9][A-Za-z0-9_.-]*))?_{DATE_TAG}\.log$"
)
RING_HEADER_RE = re.compile(r"^===== H1221 TYPE2 QUOTIENT (q\d{2}) =====$")
H0_RE = re.compile(
...
class Task:
    ...
    def canonical_log_name(self) -> str:
        return (
            f"sprime_length6_h1221_{self.ring}_{self.row.tag}_"
...
def parse_filename(path: Path) -> tuple[Task | None, str | None, str | None]:
    ...
    if not match:
        return None, None, "malformed H1221 task-log filename"
    ring, fiber, model, coordinate_text, variant = match.groups()
    if ring not in RINGS:
        return None, variant, f"ring tag {ring!r} is not one of the 19 targets"
...
def audit_log(path: Path) -> LogAudit:
    ...
    ring_gate = exactly(
        lines,
        lambda line: "[ring/direct-sum/lift/locality/type gate]" in line,
    )
    ...
    terminal_summary_positions = [
        ...
        if line.strip() == "===== H1221 TYPE2 TERMINAL SUMMARY ====="
    ]
...
def main() -> int:
    ...
    pattern = f"sprime_length6_h1221_q*_i*_{DATE_TAG}.log"
