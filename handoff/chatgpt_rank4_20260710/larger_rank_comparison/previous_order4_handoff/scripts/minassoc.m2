-- minassoc.m2 (session 3): gensearch.m2 with J = associativity + Delta-multiplicativity ONLY
-- (no coassociativity, no fiber2), per the ablation result: M+A is the minimal
-- axiom set forcing [4]^# = 0 at eps^3 for BOTH fiber shapes (scripts/ablate2.log).
-- Success here = the sharpened statement over EVERY F_2-algebra k.
...
collectEqs = data -> (
    vecs := data#"assocV" | data#"compatV";   -- MINIMAL SET M+A: no coassocV
...
runBranch = (data, name) -> (
    ...
    J := ideal apply(unique hopf, g -> toQ2 g);   -- MINIMAL SET: fiber2 NOT included
...
runBranch(data1, "fiber k[x,y]/(x^2,y^2), R = k[eps]/eps^3, J = assoc+compat ONLY")
runBranch(data2, "fiber k[t]/(t^4), R = k[eps]/eps^3, J = assoc+compat ONLY")

<< endl << "DONE minassoc" << endl;
