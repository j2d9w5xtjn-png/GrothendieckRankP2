-- z8hopf.m2 (z8search + antipode equations)
...
sIx = flatten for i from 1 to 3 list for r from 1 to 3 list s_(i,r);

Q = ZZ[ cIx | dIx | wIx | sIx ];

runZ8 = (fibKeys, name) -> (
    << endl << "########## Z/8+antipode branch: " << name << " ##########" << endl;
    Mc := (i,j,r) -> (
    ...
    P4L := for i from 1 to 3 list (
        ...
        toList out);
    -- antipode: S(e_i) = sum_r s_(i,r) e_r (S preserves augmentation),
    -- S an algebra map, and both convolution identities S*id = id*S = unit o counit
    SL := for i from 0 to 3 list (
        if i == 0 then ebas 0
        else apply(4, r -> if r == 0 then 0_Q else s_(i,r)));
    SofVec := v -> (
        out := new MutableList from toList(4:0_Q);
        for r from 0 to 3 do if v#r != 0 then (
            Si := SL#r;
            for t from 0 to 3 do if Si#t != 0 then out#t = out#t + v#r*Si#t);
        toList out);
    santiV := flatten for i from 1 to 3 list for j from i to 3 list (
        lhs := SofVec(Stab#(i,j));
        rhs := mulA(SL#i, SL#j);
        apply(4, t -> lhs#t - rhs#t));
    convV := flatten for i from 1 to 3 list (
        D := DEl#i;
        out1 := new MutableList from toList(4:0_Q);
        out2 := new MutableList from toList(4:0_Q);
        for j from 0 to 3 do for k from 0 to 3 do if D#(idx2(j,k)) != 0 then (
            u1 := mulA(SL#j, ebas k);
            u2 := mulA(ebas j, SL#k);
            for t from 0 to 3 do (
                out1#t = out1#t + D#(idx2(j,k))*u1#t;
                out2#t = out2#t + D#(idx2(j,k))*u2#t));
        (toList out1) | (toList out2));
    fiber2E := flatten for i from 1 to 3 list
        for r from 1 to 3 list ((phiL#i)#r - 2*w_(i,r));
    eqs := unique select(
        flatten(assocV | compatV | coassocV) | fiber2E | santiV | convV,
        f -> f != 0);
