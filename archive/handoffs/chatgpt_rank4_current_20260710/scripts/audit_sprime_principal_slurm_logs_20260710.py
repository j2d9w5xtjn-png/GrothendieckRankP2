#!/usr/bin/env python3
"""Strictly audit the 300-task principal-chain-ring Slurm sweep.
...
Repeated ``--log-dir`` options combine retries or banked copies.  Valid
duplicates are compared; contradictory conclusive results are errors.
...
def audit_log(
    ...
    if not match:
        return LogAudit(
            ...
            issues=["malformed principal Slurm task-log filename"],
        )
    filename_job, task_text = match.groups()
    try:
        task = Task.from_id(int(task_text))
    except ValueError as exc:
        return LogAudit(path, filename_job, None, "error", issues=[str(exc)])
    audit = LogAudit(path, filename_job, task, "error")
    ...
    if len(case_main) != 1:
        ...
        audit.slurm_job = job
        expected = (
            filename_job, str(task.task_id), task.ring,
        ...
        if observed != expected:
            audit.issues.append(
                ...
                f"expected array_job,task,ring,row,i={expected}, observed={observed}"
            )
    ...
    marker_groups = (
        ...
        ("TASK_DONE", [index for index, _ in task_done_matches]),
    )
    ...
    if len(task_done_matches) == 1:
        ...
        done_job, done_task, _ = done.groups()
        if done_task != str(task.task_id):
            audit.issues.append(
                f"TASK_DONE task mismatch: expected {task.task_id}, observed {done_task}"
            )
        if audit.slurm_job is not None and done_job != audit.slurm_job:
            audit.issues.append(
                f"TASK_DONE job mismatch: expected {audit.slurm_job}, observed {done_job}"
            )
    ...
    if not terminal_complete:
        ...
        if not audit.issues:
            audit.issues.append("missing complete driver/TASK_DONE tail")
        return audit
    if audit.issues:
        audit.state = "error"
        return audit
...
def main() -> int:
    ...
    log_dirs = args.log_dir or [Path("logs")]
    ...
    for directory in log_dirs:
        ...
        candidates.extend(
            path for path in directory.rglob("principal_[0-9]*.log")
            if path.is_file()
        )
    unique_candidates = sorted(dict.fromkeys(path.resolve() for path in candidates))
    audits = [
        audit_log(path, expected_sha, expected_manifest, expected_manifest_sha)
    ...
    print("PRINCIPAL SLURM ARRAY AUDIT")
    print("log_dirs=" + ",".join(str(path) for path in log_dirs))
    ...
    if missing_dirs:
        print("missing_log_dirs=" + ",".join(str(path) for path in missing_dirs))
    ...
    jobs = sorted({
        audit.filename_job for audit in audits
        if audit.path in selected_paths and audit.valid_terminal and audit.filename_job
    })
    if resources:
