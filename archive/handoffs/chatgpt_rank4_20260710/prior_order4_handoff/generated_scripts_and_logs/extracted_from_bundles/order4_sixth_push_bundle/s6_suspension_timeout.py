import sys
from z3 import Solver, BitVec, BitVecVal, set_param, Or, unknown
...
c1=BitVec('pin_c1_s6b',1); c4=BitVec('pin_c4_s6b',1)
def pin0(i,j,k):
    if i==1:
...
print('built',len(base),'base',len(known),'known',flush=True)
for s_idx in (2,3):
  Sh=dmat(P,s_idx,shift=1)
  ss=Solver(); ss.set('timeout',60000)
  for e in base: ss.add(e)
  for x in known: ss.add(K.eq0(x))
  ss.add(Or(*[K.neq0(v) for v in Sh.values()]))
  print('susp',s_idx,ss.check(),flush=True)
