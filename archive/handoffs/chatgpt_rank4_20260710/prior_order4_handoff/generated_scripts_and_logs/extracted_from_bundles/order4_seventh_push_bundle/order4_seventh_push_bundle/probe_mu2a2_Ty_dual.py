...
def shape_constraints_E(phi):
    cons=[]; z=kzero()
    for mon in ['U','V']:
      for r in range(1,4): cons.append(keq(coeff(phi[2][r],mon), z))
      for r in range(1,4): cons.append(keq(coeff(phi[3][r],mon), z))
      cons.append(keq(coeff(phi[1][1],mon), z)); cons.append(keq(coeff(phi[1][3],mon), z))
    return cons

def solve_Ty():
    A,M,C,F,phi,c,Mtab=build_blocks(R,fib)
    s=Solver(); s.set('timeout',600000)
    for e in A+M+C+F+pin(c,models['mu2a2'])+shape_constraints_E(phi): s.add(e)
    s.add(Or(kneq0(coeff(phi[2][1],'UV')), kneq0(coeff(phi[2][3],'UV'))))
    res=s.check(); print('mu2a2 T(y) not in ky over dual coeff',res,flush=True)
    if res==sat:
      mm=s.model();
      for name,mon in [('P','U'),('Q','V'),('T','UV')]:
        print(name)
