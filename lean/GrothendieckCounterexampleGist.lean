...
and `lambda` is group-like.  The `n`-th convolution power of the identity (the `n`-th power
word) sends `U` to `(1 + lambda + ⋯ + lambdaⁿ⁻¹) · U`.  For `n = 4` this is `2bUV ≠ 0`, while
the eighth power word is the unit word.  In particular the seventh power word supplies an
antipode, so `A` is a Hopf algebra, and the associated group scheme has order four but is not
killed by four.
...
* `Counterexample.GrothendieckPower.powerMap`: the coordinate-ring map of the `n`-th power
  word.

## Main results

* `Counterexample.GrothendieckPower.finrank_A`: `A` is free of rank four over `R`.
* `Counterexample.GrothendieckPower.powerMap_four_U_ne_zero`: the fourth power word is not
  the unit word, since it sends `U` to `2bUV ≠ 0`.
* `Counterexample.GrothendieckPower.powerMap_eight`: the eighth power word is the unit word.
* `Counterexample.GrothendieckPower.counterexample`: the combined statement.
...
The construction of this group scheme as well as the formalization were carried out by the AI assistants Codex (OpenAI) and Claude (Anthropic).
...
private theorem delta_lambda :
  ...
  obtain ⟨ha₁, hb₁, hab₁, hv₁', -⟩ := mapped_relations (S := T) left
  obtain ⟨-, -, -, hv₂', hu₂'⟩ := mapped_relations (S := T) right
  simp only [right_aA, right_bA] at hv₂' hu₂'
  ...
  QuadraticAlgebra.lift ⟨deltaV, by
    ...
    calc
      ...
      _ = (0 : R) • (1 : T) + (a ^ 2 : R) • deltaV := by
        calc
          aaT ^ 2 * deltaV = ((a ^ 2 : R) • (1 : T)) * deltaV := by rw [aaT_smul, hsquare]
